  ...
  for (var i = 0; i < liC.length; i++) {
    liC[i].onclick = function () {
      if (flag) {
        ...
        // console.log(this);
        var sec = getNext(this); //li下一个兄弟
        // console.log(sec);
        var sib = siblings(sec.parentNode); //li除了自己以外的其他亲兄弟元素
        var otherArr = []; //其他所有的二级菜单
        var arrowClass = []; //其他所有的箭头
        ...
        for (var j = 0; j < sib.length; j++) {
          ...
          for (var i = 0; i < sibSec.length; i++) {
            if (sibSec[i].className == "secondary") {
              otherArr.push(sibSec[i]); //存其他所有的secondary
            }
            if (sibSec[i].className == "arrowRot") {
              arrowClass.push(sibSec[i]); //存其他所有的箭头
            }
        ...
        // 留自己
        var selfSec = sec.children; //自己的二级菜单
        console.log(selfSec.length);
        sec.style.height = selfSec.length * 0.4 + "rem";
